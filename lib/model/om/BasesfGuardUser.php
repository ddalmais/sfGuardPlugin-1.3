...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	protected $lastTotoCriteria = null;
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->collTotos !== null) {
				foreach($this->collTotos as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getTotos() as $relObj) {
				$copyObj->addToto($relObj->copy($deepCopy));
			}
	...
	public function initTotos()
	{
		if ($this->collTotos === null) {
			$this->collTotos = array();
		}
	}

	
	public function getTotos($criteria = null, $con = null)
	{
				include_once 'lib/model/om/BaseTotoPeer.php';
		if ($criteria === null) {
		...
		if ($this->collTotos === null) {
			if ($this->isNew()) {
			   $this->collTotos = array();
				...
				$criteria->add(TotoPeer::SF_GUARD_USER_ID, $this->getId());

				TotoPeer::addSelectColumns($criteria);
				$this->collTotos = TotoPeer::doSelect($criteria, $con);
			}
		} else {
						if (!$this->isNew()) {
				...
				TotoPeer::addSelectColumns($criteria);
				if (!isset($this->lastTotoCriteria) || !$this->lastTotoCriteria->equals($criteria)) {
					$this->collTotos = TotoPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastTotoCriteria = $criteria;
		return $this->collTotos;
	}

	
	public function countTotos($criteria = null, $distinct = false, $con = null)
	{
				include_once 'lib/model/om/BaseTotoPeer.php';
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		return TotoPeer::doCount($criteria, $distinct, $con);
	}

	
	public function addToto(Toto $l)
	{
		$this->collTotos[] = $l;
